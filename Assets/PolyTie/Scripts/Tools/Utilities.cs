...
public static class Utilities
{
    ...
    {
        var state = ColorStates.IDLE;

        if (collider.rigidbody2D != null)
        {
            if (collider.rigidbody2D.isKinematic == true)
            {
                if (collider.GetComponent<PlatformMotor>() != null)
                    state = ColorStates.PLATFORM;
                else
                    state = ColorStates.STATIC;
            }
            else
            {
                state = ColorStates.DYNAMIC;
            }
        }
        if (collider.isTrigger == true)
        {
            state = ColorStates.TRIGGER;
            if (collider.GetComponent<KillingZoneController>() != null)
                state = ColorStates.KILLING;
        }
